...
import type { Kickout } from '../types/match';

// Kickout with match date for filtering
export interface KickoutWithDate extends Kickout {
  matchDate: string;
}

interface KickoutHeatmapProps {
  kickouts: KickoutWithDate[];
  teamName: string;
  onClose: () => void;
}

type HeatmapView = 'won' | 'lost' | 'all' | 'zones';
...
const calculateZoneStats = (kickouts: Kickout[]): ZoneData[][] => {
  const zones: ZoneData[][] = Array(GRID_ROWS).fill(null).map(() =>
    Array(GRID_COLS).fill(null).map(() => ({ total: 0, won: 0, lost: 0, winRate: 0 }))
  );

  kickouts.forEach(kickout => {
    const col = Math.min(Math.floor(kickout.x / 100 * GRID_COLS), GRID_COLS - 1);
    const row = Math.min(Math.floor(kickout.y / 100 * GRID_ROWS), GRID_ROWS - 1);
  ...
  zones.forEach(row => {
    row.forEach(zone => {
      zone.winRate = zone.total > 0 ? Math.round((zone.won / zone.total) * 100) : 0;
    });
  });
...
const getZoneColor = (winRate: number, total: number): string => {
  if (total === 0) return 'transparent';
  
  if (winRate >= 70) return 'rgba(34, 197, 94, 0.6)'; // Green - high success
  if (winRate >= 50) return 'rgba(234, 179, 8, 0.6)'; // Yellow - medium
  if (winRate >= 30) return 'rgba(249, 115, 22, 0.6)'; // Orange - below average
  return 'rgba(239, 68, 68, 0.6)'; // Red - low success
};

export const KickoutHeatmap = ({ kickouts, teamName, onClose }: KickoutHeatmapProps) => {
  ...
  // Filter kickouts by date range
  const dateFilteredKickouts = useMemo(() => {
    const { start, end } = getDateRange(dateRange, customStartDate, customEndDate);
    return kickouts.filter(k => {
      const matchDate = new Date(k.matchDate);
      return matchDate >= start && matchDate <= end;
    });
  }, [kickouts, dateRange, customStartDate, customEndDate]);

  // Filter kickouts based on view (won/lost/all)
  const filteredKickouts = useMemo(() => {
    switch (view) {
      case 'won':
        return dateFilteredKickouts.filter(k => k.won);
      case 'lost':
        return dateFilteredKickouts.filter(k => !k.won);
      default:
        return dateFilteredKickouts;
    }
  }, [dateFilteredKickouts, view]);

  // Calculate zone statistics based on date-filtered kickouts
  const zoneStats = useMemo(() => calculateZoneStats(dateFilteredKickouts), [dateFilteredKickouts]);
  ...
  const maxCount = useMemo(() => {
    ...
    zoneStats.forEach(row => {
      row.forEach(zone => {
        const count = view === 'won' ? zone.won : view === 'lost' ? zone.lost : zone.total;
  ...
  // Summary stats based on date-filtered kickouts
  const stats = useMemo(() => {
    const won = dateFilteredKickouts.filter(k => k.won).length;
    const lost = dateFilteredKickouts.filter(k => !k.won).length;
    const total = dateFilteredKickouts.length;
    const winRate = total > 0 ? Math.round((won / total) * 100) : 0;
    return { won, lost, total, winRate };
  }, [dateFilteredKickouts]);
      ...
      <div className="bg-gray-800 rounded-xl w-full max-w-lg max-h-[90vh] overflow-auto">
        ...
        <div className="sticky top-0 bg-gray-800 p-4 border-b border-gray-700 z-10">
          <div className="flex justify-between items-center mb-2">
            <h2 className="text-xl font-bold text-white">🎯 Kickout Heatmap</h2>
            <button
              onClick={onClose}
          ...
          <p className="text-gray-400 text-sm">{teamName} · {stats.total} kickouts</p>
        </div>
        ...
        <div className="p-4 border-b border-gray-700">
            ...
            <div className="flex gap-2 mt-2">
              <div className="flex-1">
                ...
                <input
                  ...
                  className="w-full bg-gray-700 text-white text-sm rounded px-2 py-1.5 focus:outline-none focus:ring-1 focus:ring-teal-500"
                />
              </div>
              <div className="flex-1">
          ...
          {dateRange !== 'all' && (
            <p className="text-gray-500 text-xs mt-2">
              Showing {stats.total} of {kickouts.length} kickouts
            </p>
          )}
        </div>
        ...
        <div className="flex border-b border-gray-700">
          {(['all', 'won', 'lost', 'zones'] as HeatmapView[]).map(v => (
            <button
              key={v}
              onClick={() => setView(v)}
              className={`flex-1 py-3 text-sm font-medium transition-colors ${
                view === v
                  ? 'text-white border-b-2 border-green-500 bg-gray-700/50'
        ...
        <div className="p-4">
          <div className="relative bg-green-800 rounded-lg overflow-hidden" style={{ aspectRatio: '100/120' }}>
            ...
            <svg viewBox="0 0 100 120" className="absolute inset-0 w-full h-full">
              {view === 'zones' ? (
                // Zone view with win rate colors
                  ...
                  row.map((zone, colIndex) => (
                    <g key={`${rowIndex}-${colIndex}`}>
                      <rect
                        ...
                        fill={getZoneColor(zone.winRate, zone.total)}
            ...
            {showMarkers && view !== 'zones' && (
              <svg viewBox="0 0 100 120" className="absolute inset-0 w-full h-full pointer-events-none">
                {filteredKickouts.map(kickout => (
                  <g key={kickout.id}>
                    <circle
                      cx={kickout.x}
                      cy={(kickout.y / 100) * 120}
                      r="2"
                      fill={kickout.won ? '#22c55e' : '#ef4444'}
                      stroke="white"
                      strokeWidth="0.4"
                    />
                  </g>
